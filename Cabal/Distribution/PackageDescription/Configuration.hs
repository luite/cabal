...
simplifyWithSysParams os arch info cond = (cond', flags)
  where
    (CompilerId comp compVer) = compilerInfoId info
    (cond', flags) = simplifyCondition cond interp
    interp (OS os')    = Right $ os' == os
    interp (Arch arch') = Right $ arch' == arch
    interp (Impl comp' vr)
      | comp' == comp && compVer `withinRange` vr = Right True
      | otherwise = case compilerInfoCompat info of
          -- fixme: Nothing means unknown, not empty list of compatible implementations
          Nothing -> Right False 
          Just compat -> Right $
            any (\(CompilerId c v) -> comp' == c && v `withinRange` vr) compat
