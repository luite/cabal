...
convSPI' os arch comp strfl = L.map (convSP os arch comp strfl) . CI.allPackages

convSPI :: OS -> Arch -> CompilerInfo -> Bool ->
           CI.PackageIndex SourcePackage -> Index
convSPI os arch comp strfl = mkIndex . convSPI' os arch comp strfl

-- | Convert a single source package into the solver-specific format.
convSP :: OS -> Arch -> CompilerInfo -> Bool -> SourcePackage -> (PN, I, PInfo)
convSP os arch comp strfl (SourcePackage (PackageIdentifier pn pv) gpd _ _pl) =
  let i = I pv InRepo
  in  (pn, i, convGPD os arch comp strfl (PI pn i) gpd)
...
convBranch os arch comp pi fds p (c', t', mf') =
  go c' (          convCondTree os arch comp pi fds p   t')
        (maybe [] (convCondTree os arch comp pi fds p) mf')
  where
    (CompilerId cf cv) = compilerInfoId comp
    ...
    go (Var (Impl cf' cvr')) t f
      | (cf == cf' && checkVR cvr' cv) ||
        any (\(CompilerId cf'' cv') -> cf == cf'' && checkVR cvr' cv')
            -- fixme: Nothing means unknown, not empty list of compatible impls
            (fromMaybe [] $ compilerInfoCompat comp) = t
      | otherwise      = f
